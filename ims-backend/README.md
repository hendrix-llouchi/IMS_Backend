...
> Requires `role = owner`. Access any other role → `403`.
...
> Requires `role = manager`. Access any other role → `403`.
...
The test suite uses **PHPUnit** with an **SQLite in-memory database** for full isolation.
...
- Authenticated requests use `withHeader('Authorization', "Bearer {$token}")` directly — this is required because `withCookie()` is intercepted by `EncryptCookies` middleware before the JWT can be read.
...
For detailed per-endpoint request/response documentation including all field validations and example payloads, see [`API_REFERENCE.md`](./API_REFERENCE.md).
